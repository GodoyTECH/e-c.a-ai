...
export function Storefront({
  ...
  const optionalToppings = useMemo(() => {
    ...
    const includedIds = new Set((selectedProduct.included_toppings || []).map((item) => item.topping_id));
    return toppings
      .filter((item) => item.active && !item.archived && !includedIds.has(item.id))
      .map((item) => ({ topping_id: item.id, name: item.name, price_cents: item.price_cents, active: item.active, sort_order: item.sort_order }));
